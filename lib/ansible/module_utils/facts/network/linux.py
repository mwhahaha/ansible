...
class LinuxNetwork(Network):
    ...
    def get_interface_ip_details(self, device):
        return data
    ...
    def get_interfaces_info(self, ip_path, default_ipv4, default_ipv6):
        ...
        interface_paths = glob.glob('/sys/class/net/*')

        pool = ThreadPool(processes=min(len(interface_paths), cpu_count()))
        for path in interface_path:
            if not os.path.isdir(path):
                continue
            device = os.path.basename(path)
            interfaces[device] = {}
            interfaces[device].update(pool.apply_async(self.get_interface_details, (device, path)))
            ...
            """
            interfaces[device].update(pool.apply_async(self.get_ethtool_data, (device)))
